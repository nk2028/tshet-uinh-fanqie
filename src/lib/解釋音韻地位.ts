...
// TODO: more descriptive 韻鏡位置
const 韻鏡位置to韻鏡描述 = (當前韻鏡位置: 韻鏡位置): string => {
  const { 轉號, 上位, 右位 } = 當前韻鏡位置;
  return `(轉號${轉號}, 上位${上位}, 右位${右位})`;
};
...
const 解釋字的音韻地位來源 = (字: string, 當前音韻地位: 音韻地位) => {
  ...
  defaultLogger.log(`在《韻鏡》中查得「${韻鏡位置字頭}」字${異體字處理}位於韻鏡位置 ${韻鏡位置to韻鏡描述(當前韻鏡位置)}`);
  defaultLogger.log(`![韻鏡（古逸叢書本）第 ${當前韻鏡位置.轉號} 轉](${generate韻鏡Url(當前韻鏡位置)})`);
  當前韻鏡位置.to音韻地位();
  defaultLogger.log(`故韻鏡位置 ${韻鏡位置to韻鏡描述(當前韻鏡位置)} 對應的切韻音系音韻地位為「${當前音韻地位.描述}」`);
};
