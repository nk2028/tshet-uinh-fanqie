...
export interface 上海話選項 {
  文白讀: '文上白下' | '僅白讀' | '僅文讀' | '主流層';
  標調方式: '數字調值' | '折線' | '附標' | '八調序號' | '弗標';
  分尖團: '分尖團' | '區分⟨情､琴⟩' | '區分⟨徐､齊⟩';
  分衣煙: boolean;
  分來蘭: boolean;
  分袜麦: boolean;
  分打黨: boolean;
...
const 上海話選項列表 = () => [
  ['文白讀', [上海話預設選項.文白讀, '文上白下', '僅白讀', '僅文讀', '主流層']],
  ['標調方式', [上海話預設選項.標調方式, '數字調值', '折線', '附標', '八調序號', '弗標']],
  ['分尖團', [上海話預設選項.分尖團, '分尖團', '區分⟨情､琴⟩', '區分⟨徐､齊⟩']],
  ['分衣煙|區分⟨衣､煙⟩', true],
  ['分來蘭|區分⟨來､蘭⟩', true],
  ['分袜麦|區分⟨袜､麦⟩', true],
  ['分打黨|區分⟨打､黨⟩', true],
...
function 推導上海話(選項?: Partial<上海話選項>): readonly unknown[];
function 推導上海話(選項: Partial<上海話選項>, 當前音韻地位: 音韻地位): string;
function 推導上海話(當前音韻地位: 音韻地位, 指定選項?: Partial<上海話選項>): string;
function 推導上海話(
  音韻地位或選項?: 音韻地位 | Partial<上海話選項> | null,
  選項或音韻地位: 音韻地位 | Partial<上海話選項> = {}
): string | readonly unknown[] {
  let 當前音韻地位: 音韻地位;
  let 指定選項: Partial<上海話選項>;
  ...
  const 選項 = { ...上海話預設選項, ...指定選項 };

  const 字頭 = '[<unknown>]'; // 音韻地位由反切得出，無字頭信息
  ...
  const 文讀聲母 = 聲母規則(true);
  const 白讀聲母 = 聲母規則(false);

  const 文讀韻母 = 韻母規則(true);
  const 白讀韻母 = 韻母規則(false);
