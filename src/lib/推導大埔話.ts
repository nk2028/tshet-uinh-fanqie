// branched from https://github.com/nk2028/tshet-uinh-examples/blob/59142e340364947aa5089277185ba524775a670e/taibu.js
...
interface 大埔話選項 extends Record<string, unknown> {
  文白讀: string;
  音標: string;
  標調方式: string;
}
...
const 大埔話預設選項 = new 推導設定(大埔話選項列表).選項 as 大埔話選項;

export const 推導大埔話 = (
  音韻地位: 音韻地位,
  指定選項: Readonly<Record<string, unknown>> = {}
): string => {
  const 選項 = { ...大埔話預設選項, ...指定選項 } as 大埔話選項;
  const is = (...x: Parameters<音韻地位['屬於']>) => 音韻地位.屬於(...x);
  ...
  function 聲母規則(白讀: boolean): string {
    if (is`幫母`) {
  ...
  function 韻母規則(白讀: boolean): string {
    // 通攝
  ...
  function 聲調規則(白讀: boolean): string {
  ...
  function 聲母處理(聲母: string, 韻母: string): string {
    // 無聲母時自動補全 v
  ...
  function 韻母預先處理(韻母: string): string {
    // 替換入聲韻尾
    if (is`入聲`) {
  ...
  function 韻母善後處理(聲母: string, 韻母: string): string {
    // 先處理 i 相關韻母
  ...
  function 客拼轉IPA(讀音: 讀音): 讀音 {
    ...
    // 提取輔音韻尾
    const 輔音韻尾匹配 = 讀音.韻母.match(輔音韻尾正則匹配);
    const 輔音韻尾 = 輔音韻尾匹配?.[0] ?? '';
    // 提取元音們
    ...
    // 將元音們逐個轉換
    const 轉換後的元音列表: string[] = [];
    while (元音們.length !== 0) {
      const 當前元音 = 元音們.match(首元音正則匹配)![0];
      轉換後的元音列表.push(轉換元音[當前元音]);
      元音們 = 元音們.slice(當前元音.length);
    }
    let 轉換後的元音們 = 轉換後的元音列表.join('');
    // 轉換韻尾
  ...
  // 轉換標調方式
  function 轉換標調方式(讀音: 讀音, 標調方式: string): 讀音 {
    const 轉換結果 = 讀音;
  ...
  if (選項.文白讀 === '僅白讀') {
    ...
    if (文讀音結果 === 白讀音結果) {
      ...
      結果 = 文讀音結果 + '(文) ' + 白讀音結果 + '(白)'; // 此處將原程式碼的換行改為括號附註，便於頁面展示
    }
  }
